...
from centralHub.models import TwitterUser, SpotifyArtistInfo, SpotifyAlbumTracking
from centralHub.spotify.spotifyAPI import get_artist_albums
from centralHub.xTwitter.twitterAPI import get_tweets_since_last, XAPIUsageExceeded
            ...
            try:
                ...
                if "data" in new_posts.keys():
                    ...
                    if created_count > 0:
                        ...
                        self.stdout.write(
                            self.style.SUCCESS(
                                f"{created_count} new posts for user: '{twitter_user.twitter_name}'! Notifying..."
                            )
                        )
                else:
                    self.stdout.write(
                        self.style.ERROR(
                            f"No new posts for user {twitter_user.twitter_name}!!"
                        )
                    )
        ...
        for artist in allartists:
            ...
            for artist_album in spotify_artist_albums:
                try:
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f"Album {artist_album.name} already in db for arist, skipping"
                        )
                    )
                    continue
                if created:
            ...
            if album_count > 0:
                post_slack_message(
                    f"{album_count} new albums for artist: '{artist.spotify_artist}'! Notifying..."
                )
                self.stdout.write(
                    self.style.SUCCESS(
                        f"{album_count} new albums for artist: '{artist.spotify_artist}'"
                    )
                )
            else:
                self.stdout.write(
                    self.style.ERROR(
                        f"No new albums for user {artist.spotify_artist}!!"
                    )
                )
